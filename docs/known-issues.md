...
- スコープ外にした Jira 連携。`usecase.PRSource` と同じ形のポートを1つ足し、`adapter/jira` に実装を置けばよい
...
**計画のコード例は実行して確かめる。** 見つかった欠陥のうち 13 件は計画のサンプルコードに起因していた。計画の散文は正しい意図を述べているのに、コード例がそれを満たしていなかった。`domain` 層は計画段階で 15 テストを実行検証したので堅く、TUI のレイアウト計算と状態遷移は「コンパイルが通る」だけで済ませたため欠陥が集中した。
