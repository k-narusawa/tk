...
| `Add` は最後のチェックボックス行に続くインデント行・空行を飛ばした先に挿入する | 旧形式（詳細をインデント行で書いていたファイル）からアップグレードした直後のユーザーはその形のまま残っている。単純に「最後のチェックボックス行の直後」にすると、続くインデント行が新タスクの下にぶら下がる形に化ける。当初は `Item.Body` 復活が必要と判断してこの化けを受け入れていたが、挿入位置のヒューリスティックだけで両立できると分かり修正した |
